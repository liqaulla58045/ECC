...
async function askClaude(messageHistory, userText, systemContext) {
    ...
    try {
        ...
        return { type: 'text', text: '⚠️ Could not reach the assistant. Make sure the backend server is running.' };
    }
}
...
async function askOpenAI(userText) {
    try {
        ...
        return { type: 'text', text: '⚠️ Could not generate chart. Make sure the backend server is running.' };
    }
}
...
export default function ChatBot() {
    ...
    const handleFileChange = async (e) => {
        ...
        if (!file) return;
        const MAX = 50 * 1024;
        if (file.size > MAX) { setNotif('File too large (max 50 KB)'); return; }
        try {
...
Be concise, professional, and data-focused. Use ** for bold key terms.`;
            ...
            if (liveContext) {
                systemContext += `\n\nLive portfolio data from the PostgreSQL database:\n${liveContext}`;
            }

            if (scope === 'global') {
                systemContext += '\n\nGLOBAL SCOPE is active — provide cross-portfolio strategic insights.';
            }
                            ...
                            <div className="cb-landing-input-actions">
                                <div className="cb-landing-input-left">
                                    ...
                                    <button
                                        ...
                                        title={scope === 'global' ? 'Global scope active' : 'Switch to global scope'}
                                    >
                                        <Globe size={14} />
